...
**The journey scene.** A composed woodblock-style print sits in a gold lacquer frame at the left (`js/ui/scene.js`). It opens on the traveller setting out before **Mt Fuji**; the moment you gain a station it becomes a night **camp** — campfire, tent, pines, and a drying-rack — and everything you own is added to the picture: items hung on the rack, companions gathered by the fire (with a `+N` when there are more than fit). As your health falls the traveller grows visibly more hurt — bandages, then a wound, then a hunched lean. It re-paints from true game state on every change (`SCENE.sync`), all in one flat offline‑drawn palette, so it always matches what you're carrying and how badly you're doing. (Sound has been removed entirely.)
